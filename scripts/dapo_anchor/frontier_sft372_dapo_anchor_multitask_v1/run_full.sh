#!/usr/bin/env bash
# This is an entry point only. Creating this file does not start training.
set -euo pipefail
source "$(dirname "$0")/common.sh"

if [[ ! -f "${CALIBRATION_REPORT}" ]]; then
  echo "Calibration is required first: ${CALIBRATION_REPORT}" >&2
  exit 1
fi
mkdir -p "${LOG_ROOT}"
...
if [[ -d "${RUN_DIR}" ]] && find "${RUN_DIR}" -mindepth 1 -maxdepth 1 -print -quit | grep -q .; then
  for required in resolved_config.json runtime_signature.json recovery/latest.json; do
