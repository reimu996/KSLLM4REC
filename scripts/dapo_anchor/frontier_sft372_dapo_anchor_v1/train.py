"""DAPO-Anchor training launcher.

Reads yaml -> builds runtime_signature via
ksllm4rec_dapo_anchor._infra.dapo_fingerprint.runtime_signature -> invokes
ksllm4rec_dapo_anchor.trainer.run_training.
...
def main() -> int:
    ...
    # Load yaml -> plain dict
    config = yaml.safe_load(args.config.read_text(encoding="utf-8"))

    # Build runtime signature (all frozen inputs + code + software versions)
    from ksllm4rec_dapo_anchor._infra.dapo_fingerprint import runtime_signature
    ...
    # Kick off training
    from ksllm4rec_dapo_anchor.trainer import run_training
    ...
    try:
        result = run_training(
            ...
            aligned_cache=True,
            dense_scoring=bool(args.dense_scoring),
            formal=False,  # dapo_anchor has no SFT372_PROFILE-equivalent gate
        )
        end_event = {
