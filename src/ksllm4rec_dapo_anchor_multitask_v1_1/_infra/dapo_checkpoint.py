...
from .dapo_sampling import SourceCursor
...
class RecoveryCursor:
    ...
    def __post_init__(self) -> None:
        ...
        if self.rl_optimizer_update_step != (
            self.next_window_index * contract.OPTIMIZER_UPDATES_PER_WINDOW
        ):
            raise ValueError("RL optimizer updates must equal four per complete window.")
        if self.anchor_optimizer_update_step > max(
            0, self.next_window_index - v2_contract.ANCHOR_WARMUP_WINDOWS
        ):
            raise ValueError("At most one anchor optimizer step is allowed per completed window from W0.")
        if self.optimizer_update_step != (
            self.rl_optimizer_update_step + self.anchor_optimizer_update_step
        ):
            raise ValueError("Optimizer update total must equal the sum of RL and anchor steps.")
...
def recovery_checkpoint_due(
    ...
    *,
    epoch_finished: bool,
    interval_windows: int = 1,
) -> bool:
    if interval_windows <= 0:
        raise ValueError("interval_windows must be positive.")
    return bool(epoch_finished) or (
        cursor.next_window_index > 0
        and cursor.next_window_index % int(interval_windows) == 0
    )
...
def _cursor_from_value(value: Mapping[str, Any]) -> RecoveryCursor:
    ...
    return RecoveryCursor(
        ...
        rl_optimizer_update_step=value["rl_optimizer_update_step"],
        anchor_optimizer_update_step=value["anchor_optimizer_update_step"],
