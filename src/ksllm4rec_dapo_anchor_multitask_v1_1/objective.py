"""Shared rewards, clipped RL, and bounded GT-set anchor objectives.
...
the caller measures its raw gradient and merges a bounded contribution into
the final policy optimizer update.
"""
...
def group_relative_rewards_and_advantages(
    ...
    *,
    ...
    scored = score_group(live, positives, reward_values=reward_values)
    rewards = torch.tensor(scored.rewards, dtype=torch.float32)
    return RewardOutput(
        rewards=rewards,
        advantages=group_relative_advantages(rewards),
        mean_reward=rewards.mean(),
        std_reward=rewards.std(correction=0),
        tiers=tuple(tier.value for tier in scored.tiers),
...
def anchor_effective_lambda(
    *,
    ...
    values = {
        "max_anchor_weight": max_anchor_weight,
    ...
    if max_anchor_weight < 0.0 or target_gradient_ratio < 0.0 or epsilon <= 0.0:
    ...
    effective = min(float(max_anchor_weight), lambda_cap)
