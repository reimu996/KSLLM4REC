...
class RecoveryState:
    ...
    epoch_index: int
    next_block_in_epoch: int
    source_plan_sha256: str
    epoch_plan_sha256s: tuple[str, str]
    ...
    window_log_rows: int
    anchor_log_rows: int
    epoch_auxiliary_flushes: tuple[Mapping[str, Any], ...] = ()
    ...
    def __post_init__(self) -> None:
        ...
        if self.epoch_index > 2:
            raise ValueError("epoch_index must be in [0, 2].")
        if self.next_block_in_epoch > 532:
            raise ValueError("next_block_in_epoch must be at most 532.")
        if self.epoch_index == 2 and self.next_block_in_epoch != 0:
            raise ValueError("Completed training must use epoch_index=2, block=0.")
        if self.epoch_index < 2 and self.next_block_in_epoch == 532:
            raise ValueError("Epoch boundary checkpoints must advance the epoch cursor.")
        if self.next_source_block != self.epoch_index * 532 + self.next_block_in_epoch:
            raise ValueError("Global and epoch-local source cursors differ.")
        if not isinstance(self.source_plan_sha256, str) or not re.fullmatch(
            r"[0-9a-f]{64}", self.source_plan_sha256
        ):
            raise ValueError("source_plan_sha256 must be a SHA256 digest.")
        if (
            not isinstance(self.epoch_plan_sha256s, tuple)
            or len(self.epoch_plan_sha256s) != 2
            or any(not isinstance(value, str) or not re.fullmatch(r"[0-9a-f]{64}", value) for value in self.epoch_plan_sha256s)
        ):
            raise ValueError("epoch_plan_sha256s must contain two SHA256 digests.")
        if self.auxiliary_flush_steps > 2:
            raise ValueError("At most one epoch-end auxiliary step per epoch is allowed.")
        if self.total_optimizer_steps != (
            self.completed_policy_steps + self.auxiliary_flush_steps
        ):
            raise ValueError(
                "total_optimizer_steps must equal completed_policy_steps plus "
                "auxiliary_flush_steps."
            )
        flushes = tuple(self.epoch_auxiliary_flushes)
        if len(flushes) > 2:
            raise ValueError("At most two epoch-end flush audits are allowed.")
        if any(not isinstance(value, Mapping) for value in flushes):
            raise TypeError("epoch_auxiliary_flushes must contain mappings.")
        flush_steps = sum(
            _non_negative_integer(
                value.get("optimizer_steps"),
                "RecoveryState.epoch_auxiliary_flushes.optimizer_steps",
            )
            for value in flushes
        )
        if any(int(value.get("optimizer_steps", -1)) > 1 for value in flushes):
            raise ValueError("Each epoch-end flush permits at most one step.")
        if flush_steps != self.auxiliary_flush_steps:
            raise ValueError("Epoch auxiliary audits and optimizer-step counters differ.")
...
def _progress_value(state: RecoveryState) -> dict[str, Any]:
    ...
    return {
        ...
        "next_block_in_epoch": state.next_block_in_epoch,
        "source_plan_sha256": state.source_plan_sha256,
        "epoch_plan_sha256s": list(state.epoch_plan_sha256s),
        ...
        "anchor_log_rows": state.anchor_log_rows,
...
def _state_from_values(progress: Mapping[str, Any], payload: Any) -> RecoveryState:
    ...
    return RecoveryState(
        ...
        next_block_in_epoch=progress["next_block_in_epoch"],
        source_plan_sha256=progress["source_plan_sha256"],
        epoch_plan_sha256s=tuple(progress["epoch_plan_sha256s"]),
        ...
        auxiliary_flush_steps=progress["auxiliary_flush_steps"],
        ...
        anchor_log_rows=progress["anchor_log_rows"],
        epoch_auxiliary_flushes=tuple(progress["epoch_auxiliary_flushes"]),
    )
