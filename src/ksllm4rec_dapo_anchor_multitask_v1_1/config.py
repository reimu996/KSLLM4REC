"""Frozen config builder for DAPO-Anchor-Multitask V1.1 exact two-epoch training."""
...
def _log_dir_for_arm(arm: str) -> Path:
    return (
        ...
        / f"operation_logs/rloo/dapo_anchor_multitask_sft372_v1_1_e2{_arm_suffix(arm)}"
    )
...
def _run_dir_for_arm(arm: str) -> Path:
    return (
        contract.PROJECT_ROOT
        / f"artifacts/rloo/runs/dapo_anchor_multitask_sft372_v1_1_e2{_arm_suffix(arm)}"
    )


_LOG_DIR = _log_dir_for_arm("D")
_RUN_DIR = _run_dir_for_arm("D")
...
def _arm_contract(arm: str) -> dict[str, Any]:
    ...
    return {
        ...
        "text_to_sid_enabled": arm in {"C", "D"},
        "reward_profile": "old" if arm in {"A", "D"} else "new",
    }
...
_CONFIG_TEMPLATE: dict[str, Any] = {
    ...
    "sampling": {
        ...
        "shuffle": "sha256_seed_epoch_task_group_id",
        "epoch_boundary_flush": True,
    },
    "reward": dict(contract.OLD_REWARD_VALUES),
    "loss": {
    ...
    "anchor": {
        ...
        "routes": ["all_normalized_groups"],
        "coverage": "all_normalized_groups_once_per_epoch",
...
def build_config(
    *,
    ...
    reward = (
        contract.OLD_REWARD_VALUES
        if arm in {"A", "D"}
        else contract.NEW_REWARD_VALUES
    )
...
def validate_config(
    ...
    if config.get("schema_version") != contract.SCHEMA_VERSION:
        raise ValueError("Expected schema_version 4.")
    if config.get("spec_version") != contract.SPEC_VERSION:
        raise ValueError("Expected DAPO-Anchor-Multitask V1.1-E2.")
    if config.get("profile") != contract.PROFILE:
        raise ValueError("Unexpected Multitask V1.1 profile.")
    active = config.get("experiments", {}).get("active", {})
    if config.get("experiments", {}).get("arms") != list(contract.EXPERIMENT_ARMS):
        raise ValueError("experiments.arms differs from the frozen arm matrix.")
    arm = active.get("arm")
    ...
    expected = {
        ...
        ("sampling", "shuffle"): "sha256_seed_epoch_task_group_id",
        ("sampling", "epoch_boundary_flush"): True,
        ...
        ("anchor", "routes"): ["all_normalized_groups"],
        ("anchor", "coverage"): "all_normalized_groups_once_per_epoch",
        ("anchor", "target_gradient_ratio"): contract.ANCHOR_TARGET_GRADIENT_RATIO,
        ("train", "source_epochs"): contract.SOURCE_EPOCHS,
    ...
    if not allow_output_override and Path(config["output"]["run_dir"]).resolve() != (
        ...
        raise ValueError("Formal Multitask V1.1 run_dir was overridden.")
    if Path(config["output"]["log_dir"]).resolve() != _log_dir_for_arm(
        str(arm)
    ).resolve():
        raise ValueError("Formal Multitask V1.1 log_dir was overridden.")
    old_run = (
        contract.PROJECT_ROOT
        / "artifacts/rloo/runs/dapo_anchor_sft372_v2_2_2effective_epochs"
    ).resolve()
    if Path(config["output"]["run_dir"]).resolve() == old_run:
        raise ValueError("Multitask V1.1 may not write into the V2.2 run directory.")
