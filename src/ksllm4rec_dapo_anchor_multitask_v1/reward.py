...
def route_group(reward: GroupReward) -> RouteDecision:
    """Route a sampled group to RL, Anchor, both, or neither."""

    if reward.has_exact:
        if reward.effective:
            return RouteDecision(ObjectiveRoute.RL, use_rl=True, use_anchor=False)
        return RouteDecision(ObjectiveRoute.SKIP, use_rl=False, use_anchor=False)
    if reward.effective:
