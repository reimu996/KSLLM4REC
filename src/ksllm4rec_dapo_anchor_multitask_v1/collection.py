...
class PreparedGroup:
    ...
    def identity(self) -> tuple[str, str]:
        return self.group.task.value, self.group.group_id
...
class PendingOptimizationBuffer:
    ...
    def append(self, block: BlockCollection) -> "PendingOptimizationBuffer":
        ...
        audit_ids = [(item.task.value, item.source_index) for item in audits]
...
def _prepared_identity(item: Any) -> tuple[str, str]:
    ...
    return task, str(item.group.group_id)
...
def sample_source_block(
    ...
    *,
    ...
    task_inputs: list[tuple[SidTask, tuple[SidTargetGroup, ...], range]] = []
    if bool(active["recommendation_enabled"]):
        task_inputs.append(
            (
                SidTask.RECOMMENDATION,
                source_block.recommendation.select(recommendation_groups),
                range(
                    source_block.recommendation.start,
                    source_block.recommendation.stop,
                ),
            )
        )
    if bool(active["text_to_sid_enabled"]):
        task_inputs.append(
            (
                ...
                source_block.text_to_sid.select(text_to_sid_groups),
                range(source_block.text_to_sid.start, source_block.text_to_sid.stop),
            )
        )
    ...
    for task, groups, indices in task_inputs:
        ...
        for audit in task_audits:
            identity = (audit.task.value, audit.group_id)
            item = by_identity.get(identity)
            if audit.route in {ObjectiveRoute.RL, ObjectiveRoute.RL_AND_ANCHOR}:
                if item is None:
