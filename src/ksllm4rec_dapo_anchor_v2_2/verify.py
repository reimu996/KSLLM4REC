...
def validate_window_log_row(row: Mapping[str, Any]) -> None:
    ...
    calibrated = float(row["lambda_calibrated"])
    ...
    if raw > 0.0 and rl_reference > 0.0:
        ...
        expected_effective = min(calibrated, expected_cap)
        ...
        if not math.isclose(
            ...
            raise ValueError("lambda_effective is not min(calibrated,cap).")
        if not math.isclose(
...
def validate_group_log_row(row: Mapping[str, Any]) -> None:
    required_keys = {"window_index", "group_id", "effective", "gt_injection_count", "rewards"}
    missing = required_keys - set(row)
    if missing:
