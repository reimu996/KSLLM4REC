...
def discover_runtime_code_files(
    ...
    paths: list[Path] = []
    package_root = root / "src" / "ksllm4rec_dapo_anchor_v2_2"
    if package_root.is_dir():
        paths.extend(package_root.rglob("*.py"))
    script_root = root / "scripts" / "dapo_anchor" / "frontier_sft372_dapo_anchor_v2_2"
    ...
    suite_root = root / "tests" / "dapo_anchor_v2_2"
...
def runtime_signature(
    ...
    *,
    ...
    project_root: Path = _PROJECT_ROOT,
    runtime_code_files: Iterable[str | Path] | None = None,
    include_anchor_calibration: bool = True,
) -> dict[str, Any]:
    ...
    inputs = {
        ...
        **(
            {
                "anchor_calibration": _file(
                    Path(config["anchor"]["calibration_path"])
                )
            }
            if include_anchor_calibration
            else {}
        ),
        **(
            {
...
def calibration_base_signature_sha256(signature: dict[str, Any]) -> str:
    """Remove the calibration artifact from a final training signature."""

    validated = validate_runtime_signature(signature)
    inputs = dict(validated["inputs"])
    if "anchor_calibration" not in inputs:
        return validated["sha256"]
    inputs.pop("anchor_calibration")
