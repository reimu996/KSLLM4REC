...
class CalibrationWindowResult:
    window_index: int
    rl_minibatch_gradient_norms: tuple[float, ...]
    rl_reference_gradient_norm: float
    anchor_raw_gradient_norm: float
    anchor_candidate_groups: int
    lambda_window: float | None
...
def is_anchor_candidate(reward: RewardOutput) -> bool:
    """A candidate is reward-flat and contains no exact rollout slot."""

    return (not reward.effective) and all(tier != "exact" for tier in reward.tiers)
    ...
    while len(retained) < target:
        ...
        for group, proposal in zip(raw_groups, proposals, strict=True):
            ...
            if not reward.effective:
                ...
                if is_anchor_candidate(reward):
                    anchor_candidate_ids.append(group.group_id)
        ...
        audits.append(
            RawBatchAudit(
                ...
                anchor_candidate_group_ids=tuple(anchor_candidate_ids),
...
def build_anchor_raw_gradient(
    ...
    *,
    ...
    ids = tuple(item.group.group_id for item in candidates)
    if len(set(ids)) != len(ids):
    ...
    # Stable length bucketing fills eight heterogeneous rows while reducing pad.
    ordered = tuple(
        sorted(
            rows,
            key=lambda row: (
...
def run_anchor_phase(
    ...
    *,
    ...
    if not 0.0 <= float(lambda_calibrated) <= float(config["anchor"]["max_weight"]):
        raise ValueError("Calibrated anchor lambda is outside [0,max_weight].")
    ...
    if budget.update_allowed:
        ...
        if float(lambda_calibrated) <= 0.0:
            skip_reason = "zero_calibrated_lambda"
        elif float(rl_reference_gradient_norm) <= 0.0:
...
def _trainable_parameter_digest(bundle: PolicyModel) -> str:
    digest = hashlib.sha256()
    for name, parameter in sorted(bundle.model.named_parameters()):
...
def validate_calibration_report(
    ...
    *,
    base_runtime_signature_sha256: str,
) -> float:
    """Recompute the calibrated lambda from a zero-update calibration report."""

    required = {
        "schema_version",
        "spec_version",
        "base_runtime_signature_sha256",
        "requested_windows",
        "valid_windows",
        "windows",
        "lambda_calibrated",
    ...
    if missing:
        raise ValueError(f"Calibration report missing keys: {sorted(missing)}")
    if report["schema_version"] != 2 or report["spec_version"] != contract.SPEC_VERSION:
        raise ValueError("Calibration report schema/spec version is not V2.2.")
    if report["base_runtime_signature_sha256"] != base_runtime_signature_sha256:
        raise RuntimeError("Calibration report belongs to different frozen inputs/code.")
    requested = int(config["anchor"]["calibration_windows"])
    minimum = int(config["anchor"]["min_calibration_windows"])
    if int(report["requested_windows"]) != requested or len(report["windows"]) != requested:
        raise ValueError("Calibration report has the wrong window count.")
    if int(report["optimizer_updates"]) != 0:
        raise ValueError("Calibration must not perform an optimizer update.")
    if report["parameter_digest_before"] != report["parameter_digest_after"]:
        raise RuntimeError("Calibration changed trainable parameters.")

    target = float(config["anchor"]["target_gradient_ratio"])
    lambda_windows: list[float] = []
    for window in report["windows"]:
        norms = tuple(float(value) for value in window["rl_minibatch_gradient_norms"])
        if len(norms) != contract.OPTIMIZER_UPDATES_PER_WINDOW:
            raise ValueError("Each calibration window must contain four RL norms.")
        rl_reference = float(window["rl_reference_gradient_norm"])
        if not math.isclose(rl_reference, float(median(norms)), rel_tol=1.0e-9, abs_tol=1.0e-12):
            raise ValueError("Calibration RL reference is not the four-norm median.")
        anchor_raw = float(window["anchor_raw_gradient_norm"])
        recorded = window["lambda_window"]
        if rl_reference > 0.0 and anchor_raw > 0.0:
            expected = target * rl_reference / anchor_raw
            if recorded is None or not math.isclose(
                float(recorded), expected, rel_tol=1.0e-9, abs_tol=1.0e-12
            ):
                raise ValueError("Calibration per-window lambda is inconsistent.")
            lambda_windows.append(expected)
        elif recorded is not None:
            raise ValueError("A zero-signal calibration window must not define lambda.")
    if len(lambda_windows) != int(report["valid_windows"]) or len(lambda_windows) < minimum:
        raise RuntimeError(
            f"Calibration needs at least {minimum} windows with both gradients."
        )
    resolved = min(
        float(config["anchor"]["max_weight"]), float(median(lambda_windows))
    )
    if not math.isclose(
        float(report["lambda_calibrated"]), resolved, rel_tol=1.0e-9, abs_tol=1.0e-12
    ):
        raise ValueError("Calibration report lambda_calibrated is inconsistent.")
    return resolved


def load_calibrated_lambda(
    config: Mapping[str, Any], runtime_signature: Mapping[str, Any]
) -> float:
    from ._infra.dapo_fingerprint import calibration_base_signature_sha256

    path = Path(config["anchor"]["calibration_path"])
    if not path.is_file():
        raise FileNotFoundError(
            f"V2.2 training requires a completed calibration report: {path}"
        )
    try:
        report = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise RuntimeError(f"Calibration report is unreadable: {path}") from exc
    if not isinstance(report, Mapping):
        raise ValueError("Calibration report must be a JSON object.")
    return validate_calibration_report(
        config,
        report,
        base_runtime_signature_sha256=calibration_base_signature_sha256(
            runtime_signature
        ),
    )
...
def run_calibration(
    ...
    *,
    ...
    """Measure 16 unchanged-policy windows and atomically resolve lambda."""
    ...
    if output.exists():
        ...
        validate_calibration_report(
            config,
            report,
            base_runtime_signature_sha256=base_runtime_signature["sha256"],
        )
        return dict(report)
    ...
    stream = DeterministicGroupStream(
        groups,
        seed=int(config["train"]["seed"]),
        cursor=SourceCursor(cycle_index=0, offset=0),
    )
    before = _trainable_parameter_digest(bundle)
    started = time.perf_counter()
    window_results: list[CalibrationWindowResult] = []

    for window_index in range(int(config["anchor"]["calibration_windows"])):
        ...
        for minibatch in minibatches:
            ...
            rl_norms.append(step.gradient_norm)
            optimizer.zero_grad(set_to_none=True)
        rl_reference = float(median(rl_norms))
        anchor_raw = 0.0
        if collection.anchor_candidates:
            raw = build_anchor_raw_gradient(
                bundle,
                optimizer,
            ...
            anchor_raw = raw.raw_gradient_norm
            optimizer.zero_grad(set_to_none=True)
        lambda_window = (
            float(config["anchor"]["target_gradient_ratio"])
            * rl_reference
            / anchor_raw
            if rl_reference > 0.0 and anchor_raw > 0.0
            else None
        ...
        result = CalibrationWindowResult(
            window_index=window_index,
            rl_minibatch_gradient_norms=tuple(rl_norms),
            rl_reference_gradient_norm=rl_reference,
            anchor_raw_gradient_norm=anchor_raw,
        ...
        print(
            json.dumps(
                {"event": "anchor_calibration_window", **asdict(result)},
                separators=(",", ":"),
                allow_nan=False,
            ),
            flush=True,
    ...
    after = _trainable_parameter_digest(bundle)
    valid = [
        result.lambda_window
        for result in window_results
        if result.lambda_window is not None
    ]
    minimum = int(config["anchor"]["min_calibration_windows"])
    if len(valid) < minimum:
        raise RuntimeError(
            f"Calibration produced {len(valid)} valid windows; need at least {minimum}."
        )
    lambda_calibrated = min(
        float(config["anchor"]["max_weight"]), float(median(valid))
    )
    report = {
        "schema_version": 2,
        "spec_version": contract.SPEC_VERSION,
        "base_runtime_signature_sha256": base_runtime_signature["sha256"],
        "requested_windows": len(window_results),
        "valid_windows": len(valid),
        "windows": [asdict(result) for result in window_results],
        "lambda_calibrated": lambda_calibrated,
        "optimizer_updates": 0,
...
def _window_log_row(
    ...
    *,
    ...
    return {
        ...
        "lambda_calibrated": training.anchor.lambda_calibrated,
        "lambda_cap": training.anchor.lambda_cap,
...
def run_one_window_pilot(
    ...
    *,
    ...
    """Exercise W0 without creating a formal recovery point."""

    validate_runtime_signature(runtime_signature)
    lambda_calibrated = load_calibrated_lambda(config, runtime_signature)
    output = Path(output_dir).expanduser().resolve()
    _prepare_run_directory(output, config, runtime_signature, resume=False)
    configure_deterministic_runtime(int(config["train"]["seed"]))
    groups, trie = load_groups_and_trie(config)
    bundle = load_policy_model(config, device=device)
    bundle.model.train()
    ...
    logical_window = 0
    scheduler = WindowLRScheduler(
        optimizer, config, completed_windows=logical_window
    )
    stream = DeterministicGroupStream(
        groups,
        seed=int(config["train"]["seed"]),
    ...
    peak = max(collection_peak, training_peak)
    if peak > float(config["memory"]["max_reserved_gib"]):
        raise RuntimeError(f"Pilot reserved memory {peak:.3f} GiB exceeds limit.")
    _assert_optimizer_state_step(optimizer, training.optimizer_updates)
    after = _trainable_parameter_digest(bundle)
    if before == after:
        raise RuntimeError("Pilot completed optimizer steps but parameters did not change.")

    window = _window_log_row(
        collection,
        training,
        collection_seconds=collection_seconds,
        training_seconds=training_seconds,
        peak_reserved_gib=peak,
        collection_peak_reserved_gib=collection_peak,
        training_peak_reserved_gib=training_peak,
        rl_optimizer_update_step=training.rl_optimizer_steps,
        anchor_optimizer_update_step=training.anchor_optimizer_steps,
        optimizer_update_step=training.optimizer_updates,
    )
    from .verify import validate_window_log_row

    validate_window_log_row(window)
    save_policy_atomic(bundle, output / "pilot-adapter")
    report = {
        "schema_version": 2,
        "spec_version": contract.SPEC_VERSION,
        "passed": True,
        "formal_recovery_point": False,
        "logical_window_index": logical_window,
        "parameter_digest_before": before,
        "parameter_digest_after": after,
