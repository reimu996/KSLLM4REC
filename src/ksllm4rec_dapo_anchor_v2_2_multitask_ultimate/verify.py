...
def validate_window_log_row(row: Mapping[str, Any]) -> None:
    ...
    if ratio > contract.ANCHOR_TARGET_GRADIENT_RATIO + 1.0e-12:
        raise ValueError("anchor_to_rl_grad_ratio exceeds the pre-clip budget.")
    ...
    if raw > 0.0 and rl_reference > 0.0:
        expected_effective = contract.ANCHOR_TARGET_GRADIENT_RATIO * rl_reference / raw
        if not math.isclose(
            effective_lambda, expected_effective, rel_tol=1.0e-6, abs_tol=1.0e-12
        ):
            raise ValueError("lambda_effective is inconsistent with raw anchor/RL gradients.")
        if not math.isclose(
    ...
    total_decision_tokens = sum(int(step["decision_tokens"]) for step in steps)
    if total_decision_tokens != contract.SID_LAYER_COUNT * contract.EFFECTIVE_GROUPS_PER_WINDOW:
        raise ValueError(
            f"Window must carry {contract.SID_LAYER_COUNT * contract.EFFECTIVE_GROUPS_PER_WINDOW} "
            f"decision tokens (SID_LAYER_COUNT per candidate), got {total_decision_tokens}."
        )
