...
Cross-check (must all still hold):
    from ksllm4rec_dapo_anchor_v2_2 import contract as c
    from ksllm4rec_dapo_anchor_v2_2._infra import dapo_contract as legacy
...
SCHEMA_VERSION = 2
SPEC_VERSION = "DAPO-ANCHOR-V2.2-MULTITASK-E2"
PROFILE = "frontier_sft372_dapo_anchor_v2_2_multitask_e2"
...
EXPECTED_TEXT_TO_SID_GROUPS = 10_597
EXPECTED_TEXT_TO_SID_POSITIVE_EDGES = 10_597
...
# ── Anchor 常量 ──────────────────────────────────────────────────────
# V2.2 enables the anchor from W0 and uses all eligible groups in one
# independent optimizer update.  The
# dense teacher-forcing microbatch only bounds activation memory; it is not an
# optimizer batch and never samples or caps groups.
ANCHOR_WARMUP_WINDOWS = 0
ANCHOR_TARGET_GRADIENT_RATIO = 0.30
...
# ── 逐层 reward 增量(累加 = 原五档 tier)────────────────────────────
SID_LAYER_COUNT = 4
SID_LAYER_REWARD_DOMAIN = 0.01   # L1 命中 domain  (= same_domain tier)
SID_LAYER_REWARD_A = 0.14        # L2 命中 A       (same_a 0.15 − 0.01)
SID_LAYER_REWARD_B = 0.25        # L3 命中 B       (same_ab 0.40 − 0.15)
SID_LAYER_REWARD_C = 0.60        # L4 命中 C(叶子) (exact 1.00 − 0.40)
SID_LAYER_REWARD_MISS = 0.00     # 任意层偏离

# ── Epoch 定义 ───────────────────────────────────────────────────────
EFFECTIVE_EPOCHS = 2
TOTAL_MULTITASK_GROUPS = EXPECTED_RECOMMEND_GROUPS + EXPECTED_TEXT_TO_SID_GROUPS
WINDOWS_PER_EFFECTIVE_EPOCH = 863
TOTAL_WINDOWS = EFFECTIVE_EPOCHS * WINDOWS_PER_EFFECTIVE_EPOCH  # 1726
TOTAL_RL_OPTIMIZER_UPDATES = TOTAL_WINDOWS * OPTIMIZER_UPDATES_PER_WINDOW  # 6904
# At most one additional anchor step per complete window, starting at W0.
MAX_TOTAL_OPTIMIZER_UPDATES = (
    TOTAL_RL_OPTIMIZER_UPDATES + TOTAL_WINDOWS - ANCHOR_WARMUP_WINDOWS
)  # 8630
WARMUP_WINDOWS = 10
