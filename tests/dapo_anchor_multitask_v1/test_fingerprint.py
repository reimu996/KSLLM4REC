...
class MultitaskFingerprintTest(unittest.TestCase):
    def make_signature_inputs(self, root: Path) -> tuple[dict, dict[str, Path]]:
        ...
        paths = {
            ...
            "calibration": root / "anchor_calibration.json",
        ...
        paths["calibration"].write_text('{"lambda":0.01}\n', encoding="utf-8")
        paths["config_file"].write_text("spec: multitask-v1\n", encoding="utf-8")
        runtime = root / "src/ksllm4rec_dapo_anchor_multitask_v1/trainer.py"
        runtime.parent.mkdir(parents=True)
        runtime.write_text("VALUE = 1\n", encoding="utf-8")
        script = (
            root
            / "scripts/dapo_anchor/frontier_sft372_dapo_anchor_multitask_v1/train.py"
        )
        script.parent.mkdir(parents=True)
        script.write_text("VALUE = 1\n", encoding="utf-8")
        status_script = script.parent / "status.sh"
        status_script.write_text("exit 0\n", encoding="utf-8")
        test_file = root / "tests/dapo_anchor_multitask_v1/test_trainer.py"
        ...
        config = {
            "schema_version": 3,
            "profile": "frontier_sft372_dapo_anchor_multitask_v1",
            "model": {
                "base_model": str(base),
            ...
            "anchor": {"calibration_path": str(paths["calibration"])},
            "output": {
    ...
    def get_signature(
        ...
        paths: dict[str, Path],
        *,
        include_anchor_calibration: bool,
    ) -> dict:
        with patch(
            "ksllm4rec_dapo_anchor_multitask_v1._infra.dapo_fingerprint.software_versions",
            ...
            return runtime_signature(
                ...
                project_root=root,
                include_anchor_calibration=include_anchor_calibration,
    ...
    def test_discovery_scans_only_the_new_package_scripts_and_tests(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            expected = {
                root / "src/ksllm4rec_dapo_anchor_multitask_v1/trainer.py",
                root
                / "scripts/dapo_anchor/frontier_sft372_dapo_anchor_multitask_v1/train.py",
                root
                / "scripts/dapo_anchor/frontier_sft372_dapo_anchor_multitask_v1/status.sh",
                root / "tests/dapo_anchor_multitask_v1/test_trainer.py",
            }
            forbidden = {
    ...
    def test_signature_changes_when_any_required_input_changes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            config, paths = self.make_signature_inputs(root)
            before = self.get_signature(
                root, config, paths, include_anchor_calibration=True
            )
            ...
            changed_config["schema_version"] = 4
            after = self.get_signature(
                root, changed_config, paths, include_anchor_calibration=True
            )
            self.assertNotEqual(before["sha256"], after["sha256"])

    def test_calibration_is_an_explicit_optional_binding(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            config, paths = self.make_signature_inputs(root)
            base = self.get_signature(
                root, config, paths, include_anchor_calibration=False
            )
            final = self.get_signature(
                root, config, paths, include_anchor_calibration=True
            )

            self.assertNotIn("anchor_calibration", base["inputs"])
            self.assertIn("anchor_calibration", final["inputs"])
            self.assertEqual(
                calibration_base_signature_sha256(final), base["sha256"]
            )

            paths["calibration"].unlink()
            without_file = self.get_signature(
                root, config, paths, include_anchor_calibration=False
            )
            self.assertEqual(without_file["sha256"], base["sha256"])
